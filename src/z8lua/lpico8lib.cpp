...
static lua_Number sin_helper(lua_Number x) {
    ...
    int a = ((x.bits() & 0x4000 ? ~x : x).bits() & 0x3fff) + 2;
    lua_Number ret = lua_Number::frombits((a >> 2 << 4) + sintable[a >> 2]);
...
static int pico8_atan2(lua_State *l) {
    ...
    if (x) {
        ...
        int64_t  q = (std::abs(int64_t(y.bits())) << 16) / std::abs(int64_t(x.bits()));
        if (q > 0x10000)
...
static int pico8_split(lua_State *l) {
    ...
    bool convert = lua_isnone(l, 3) || lua_toboolean(l, 3);
